...
def price(S0, K, r, sigma, T, M, option="european", kind="call",
         epsilon_target=0.01, alpha=0.05, shots=4096, seed=7, use_hamming=False):
    ...
    if use_hamming:
        ...
        ST_by_weight = hamming.terminal_values_by_weight(S0=S0, K=K, r=r, sigma=sigma,
                                                         T=T, M=M)
        payoff = np.maximum(ST_by_weight - K, 0.0) if kind == "call" \
            else np.maximum(K - ST_by_weight, 0.0)
        Cmax = float(payoff.max()) * 1.0001 if payoff.max() > 0 else 1.0
        qc, target = hamming.payoff_amplitude_circuit_hamming(M, angles, payoff, Cmax)
    else:
