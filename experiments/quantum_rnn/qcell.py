...
def make_token_layer(n_qubits=4, n_layers=2):
    """Return a `qml.qnn.TorchLayer` taking a token embedding of shape (..., n_qubits)
    (one angle per wire) and returning (..., n_qubits) expectation values in [-1, 1].
    Used as the variational Q/K/V projection in the quantum self-attention block."""
    dev = qml.device("default.qubit", wires=n_qubits)
